function [hIm, ww] = L1SR_Support(lIm, zooming, patch_size, overlap, Dh, Dl, lambda, regres)
...
%         XX = sum(y'.*y', 2);        
%         SX = sum(Dl'.*Dl', 2);
%         D  = repmat(XX, 1, nbase)-2*y'*Dl+repmat(SX', nframe, 1); % Calculate the distance between the input LR image patch and the LR training image patches at position£¨i,j£©
%         [v index] = sort(D);  
%         
%         neighborhoods = index(1:1022);
%         Dl = Dl(:,neighborhoods);
%         Dh = Dh(:,neighborhoods);    
%         w = feature_sign(Dl, y, lambda);
        ...
        switch regres,
            ...
            case 'L2'
                ...
                lsups = Dl(:, idx);
                hsups = Dh(:, idx);
                w = inv(lsups'*lsups+0.8*eye(size(lsups,2),size(lsups,2)))*lsups'*mpatchfea;
                hpatch = hsups*w;
            otherwise
